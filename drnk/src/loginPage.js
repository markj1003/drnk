...
class LoginButtons extends IdReact {
    render() {
      ...
      <input type="button" className="login-button" 
        value="Log in!" onClick={() => this.props.handler({type: 'log-in'})}></input>
      <input type="button" className="login-button" 
        value="Forgot your password?" onClick={() => this.props.handler({type: 'reset'})}></input>
      <input type="button" className="login-button" 
        value="Don't have an account?" onClick={() => this.props.handler({type: 'new'})}></input></div>)
    }
}
